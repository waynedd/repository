...
function showCurrentPage(data, timeStamp) {
  ...
  for(var i = 0 ; i < data.length ; i++ ) {
    // set doi
    if( data[i].doi != '' && data[i].doi.length != 0 )
 ...
 * @param page: the total number of pages
 * @param count: the maximum page number to show
 */
...
function getFullPublication( jn ) {
  ...
  if (abbr != '' && abbr != 'Phd' && abbr != 'Book' && abbr != 'Tech' && abbr != 'Computer' && abbr != 'Software')
    booktitle += ' (' +  abbr + ')' ;
